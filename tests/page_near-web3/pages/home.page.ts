...
export class HomePage extends BasePage {
    ...
    constructor(page: Page) {
        ...
        this.firstDropdownArrow = page.locator('form').getByText('NEAR', { exact: true })
        this.secondDropdownArrow = page.locator('form').getByText('REF', { exact: true })
        this.amountInputField = page.getByPlaceholder('0.0')
        this.swapButton = page.getByRole('button', { name: 'Swap' })
        this.popUpConfirmTransactionButton = page.getByRole('button', { name: 'Confirm transaction' })
        this.balance = page.getByText('Balance:')
        this.successNotificationTitle = page.getByText('Success!')
        this.successNotificationCloseButton = page.getByRole('button', { name: 'Close' })
    }
    ...
    async clickSwapButton() {
        const messageOnFail = '"Swap" button is disabled, while it should be enabled'
        await expect(this.swapButton, messageOnFail).toBeEnabled()
        await this.swapButton.click()
    }

    async confirmTransactionPopup() {
        const messageOnFail = '"Confirm transaction" in transaction overview pop-up not visible'
    ...
    async confirmTransactionWasCorrect(balanceBeforeString: string, balanceAfterString: string, transfer: number) {
    ...
    async confirmSwapButtonNotAvailable() {
        const messageOnFail = '"Swap" button is enabled, while it should be disabled'
        await expect(this.swapButton, messageOnFail).toBeDisabled()
    }

    async restoreToDefaultTokens(tokenFrom: string, tokenTo: string) {
        await this.page.locator('form').getByText(tokenFrom, { exact: true }).click()
